...
struct Node {
    ...
    uint256[] keys; // The keys sharing the value of the node.
    mapping(uint256 => uint256) keyMap; // Maps the keys to their index in `keys`.
}
...
struct Tree {
    ...
    mapping(uint256 => uint256) keyToValue; // Maps key to its value.
}
...
library RedBlackBinaryTree {
    ...
    /** @dev Returns the value in `_key`.
     *  @param _self The tree to search in.
     *  @param _key The key of the value to return.
     *  @return Return value in `_key`.
     */
    function value(Tree storage _self, uint256 _key) public view returns (uint256) {
     ...
     */
    function keyExists(Tree storage _self, uint256 _key) public view returns (bool) {
    ...
    function isNotEmpty(Tree storage _self) public view returns (bool) {
        // TODO: is size necessary or keys.length is the same?
     ...
     */
    function remove(Tree storage _self, uint256 _key) public {
